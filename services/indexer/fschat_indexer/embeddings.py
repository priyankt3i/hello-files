...
def embed_texts(provider: dict, texts: list[str]) -> list[list[float]]:
    kind = provider.get("provider")
    if kind == "openai":
        return openai_embeddings(provider, texts)
    if kind == "azure-openai":
        return azure_openai_embeddings(provider, texts)
    if kind == "google":
        return google_embeddings(provider, texts)
    if kind == "ollama":
        return ollama_embeddings(provider, texts)
    raise ValueError(f"Provider '{kind}' is not supported for embeddings.")


def embed_query(provider: dict, text: str) -> list[float]:
    return embed_texts(provider, [text])[0]


def openai_embeddings(provider: dict, texts: list[str]) -> list[list[float]]:
    ...
    data = post_json(url, headers, payload, timeout_seconds=180)
    return [item["embedding"] for item in data["data"]]


def azure_openai_embeddings(provider: dict, texts: list[str]) -> list[list[float]]:
    ...
    data = post_json(url, headers, {"input": texts}, timeout_seconds=180)
    return [item["embedding"] for item in data["data"]]


def google_embeddings(provider: dict, texts: list[str]) -> list[list[float]]:
    ...
    data = post_json(url, {"Content-Type": "application/json"}, payload, timeout_seconds=180)
    return [item["values"] for item in data["embeddings"]]


def ollama_embeddings(provider: dict, texts: list[str]) -> list[list[float]]:
    model = provider.get("model")
    if not model:
